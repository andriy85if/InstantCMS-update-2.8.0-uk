...
function install_package(){
    ...
    if(!isFieldExists('groups', 'cover')){
        $core->db->query("ALTER TABLE `{#}groups` ADD `cover` TEXT NULL DEFAULT NULL COMMENT 'Обкладинка групи'");
    }
    ...
    if(!isFieldExists('groups', 'wall_reply_policy')){
        $core->db->query("ALTER TABLE `{#}groups` ADD `wall_reply_policy` TINYINT(1) UNSIGNED NOT NULL DEFAULT '0' COMMENT 'Політика коментування стіни' AFTER  `wall_policy`");
    }
